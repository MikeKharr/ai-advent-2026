...
```sh
node build.js            # пишет dist/graph.json (каталог в .gitignore)
...
## Чего этап 1 не делает

Vault Obsidian (этап 2), страница витрины (этап 3), `Dockerfile` и доставка
на `challenge.zpq.ai/atlas/` (этап 4). Сейчас выход один — `dist/graph.json`.
