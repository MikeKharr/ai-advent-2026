...
/**
 * Потолки размера витрины в байтах (ADR 2026-09-11-0745, раздел 5). При
...
export function sizeFindings(sizes) {
  ...
  return over.map(([key, name]) => ({
    file: 'atlas/build.js',
    line: 1,
    message: `${name} — ${(sizes[key] / KB).toFixed(1)} КБ при потолке ${LIMITS[key] / KB} КБ. Усечения нет: поднять потолок — PR класса B с LIMITS в atlas/build.js, сузить набор — решение владельца (ADR 2026-09-11-0745)`,
  }))
}
 ...
 * 2026-09-10-0550, критерий этапа 1).
 */
