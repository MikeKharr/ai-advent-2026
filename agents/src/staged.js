...
export function createStagedAgent({
  ...
  return {
     ...
     * этого агента: свой потолок ответа день 15 хранит под отдельным ключом,
     * а показывает обычным `maxTokens` (ADR 2026-09-23-0646, п. 5).
     *
     * Разворачивает ключ **только тот агент, чей потолок выше прежнего
     * предела**, — то есть тот, кому это число годно. Привязка к потолку, а
     * не к «этот агент staged», — потому что столбец общий и ключ в нём
     * есть у всех троих: агент дня 13 или 14 с этим видом перевёл бы
     * `maxTokens15` обратно в `maxTokens` и отдал своей странице 32 000,
     * которых сам же не принимает. Прежняя редакция ставила вид всем, и дни
     * 13 и 14 спасало лишь то, что их `handleProfileState` не передаёт
     * `?agent=`, — притом что их же `PUT /settings` передаёт. Защита,
     * которая держится несимметричностью внутри сданного файла, не выбрана,
     * а случайна (находка compliance к PR #218; `agent_docs/guides/
     * verification.md`, «Улика должна различать гипотезы»).
     */
    viewSettings: maxOutputTokens > LAYERED_MAX_TOKENS ? viewStagedSettings : undefined,
