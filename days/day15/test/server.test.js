// Интеграционный тест дня 15 против поддельного сервиса агентов. Устройство
...
const agent = http.createServer(async (req, res) => {
  ...
  if (settings && req.method === 'PUT') {
    ...
    // Ручка без имени агента настройки дня 15 не узнаёт: 32 000 токенов для
...
const withSession = (id = SID, pid = PID) => `day15_pid=${pid}; day15_sid=${id}`
...
test('cookie дня 13 свои: день 11 не затрагивается', async () => {
  ...
  // Запрос ушёл с идентификатором из day15_sid, а не из day11_sid.
...
test('переписка несёт живой запуск, а без диалога — null', async () => {
  ...
  const none = await call('GET', '/api/chat', undefined, { ip: '10.9.0.2', cookie: `day15_pid=${PID}` })
...
test('журнал отдаётся как CSV и не исполняется браузером', async () => {
  ...
  // страницы, и в папке загрузок оказывались одинаковые `day15-stages.csv`.
  assert.equal(
