// День 14: машина состояний дня 13 плюс память инвариантов профиля
// (ADR 2026-09-22-0827). День 13 при этом не меняется: у дня 14 свой каталог,
...
const SESSION_COOKIE = 'day14_sid'
const PROFILE_COOKIE = 'day14_pid'
...
const profileView = (profile, sessionCap) => ({
  ...
  // Инварианты профиля дня 14: их завёл человек, поэтому источника-диалога
...
async function handleStageLog(req, res, runId) {
  ...
  try {
    ...
    // `day14-stages.csv` вместо имён со временем запуска. Имя назначает
...
if (process.env.NODE_ENV !== 'test') {
  server.listen(env.PORT, () => console.log(`день 14 слушает :${env.PORT}`))
}
