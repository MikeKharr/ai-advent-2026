// Интеграционный тест дня 14 против поддельного сервиса агентов. Устройство
// унаследовано от дня 13 целиком, поэтому проверяется то же, что там, — по
// критериям приёмки ADR 2026-09-21-1747,
...
/** Настройки дня 11 на том же профиле: страница дня 14 их видеть не должна. */
...
const agent = http.createServer(async (req, res) => {
  ...
  if (settings && req.method === 'PUT') {
    ...
    // Ручка без имени агента настройки дня 14 не узнаёт: 32 000 токенов для
...
const withSession = (id = SID, pid = PID) => `day14_pid=${pid}; day14_sid=${id}`
...
test('cookie дня 13 свои: день 11 не затрагивается', async () => {
  ...
  // Запрос ушёл с идентификатором из day14_sid, а не из day11_sid.
...
test('переписка несёт живой запуск, а без диалога — null', async () => {
  ...
  const none = await call('GET', '/api/chat', undefined, { ip: '10.9.0.2', cookie: `day14_pid=${PID}` })
...
test('журнал отдаётся как CSV и не исполняется браузером', async () => {
  ...
  // страницы, и в папке загрузок оказывались одинаковые `day14-stages.csv`.
  assert.equal(
