...
// Нужны пять полей на запись: заголовок, ссылка, дата, краткое описание
// и полный текст, если издание положило его в ленту (день 3).
...
/**
 * Потолок на длину ссылки. Резать URL в промпте нельзя — белый список
 * сверяет его целиком, — поэтому запись с абсурдной ссылкой отбрасывается
 * при разборе: иначе одна сломанная лента раздувает запрос без предела.
 */
const MAX_URL = 500

/** Ниже этой длины текст из ленты — анонс, а не статья. */
const MIN_FULL_TEXT = 800
/** Потолок на статью: одна раздутая заметка не должна съесть весь бюджет. */
const MAX_FULL_TEXT = 6000
...
export function parseFeed(xml, source) {
  ...
  for (const block of blocks) {
    ...
    // Полный текст статьи, если издание его отдаёт. Порядок не случаен:
    // content:encoded — договорное место для полного текста, но Entrackr
    // кладёт его в description, а TechCrunch и Sifted не дают вовсе.
    // За страницами статей не ходим: берём только то, что опубликовано
    // для агрегаторов (ADR дня 3).
    ...
    if (href.toString().length > MAX_URL) continue
    ...
    items.push({
      ...
      // Полным считаем текст заметно длиннее анонса: у TechCrunch описание
      // на 433 символа — это тизер, а не статья.
      text: fullText.length >= MIN_FULL_TEXT ? fullText.slice(0, MAX_FULL_TEXT) : '',
      source,
    })
  }
