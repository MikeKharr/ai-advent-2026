...
export const PARAM_DEFAULTS = {
  // Ответ на широкий запрос по полным текстам длиннее дайджеста из трёх
  // пунктов, ради которого выбирались 200 в дне 2.
  maxTokens: 600,
  // Пять, а не тридцать: в день 3 в модель уходит полный текст статьи, а не
  // заголовок. Тридцать статей с источника — это ~220K токенов, больше окна
  // Haiku 4.5 и вдесятеро дороже (ADR дня 3).
  perSource: 5,
...
/**
 * Параметры запроса из формы. `prompt` — это сам запрос пользователя вместе
 * с требованиями к формату ответа (день 3); остальное — параметры генерации. Ошибка любого поля — отказ целиком:
...
export function parseParams(source, env) {
  const promptParsed = cleanText(source.prompt)
  if (!promptParsed.ok) return { ok: false, message: 'Запрос должен быть строкой' }
  const prompt = promptParsed.text
  if (prompt.length > PARAM_LIMITS.prompt) {
    return { ok: false, message: `Запрос: не больше ${PARAM_LIMITS.prompt} символов` }
  }
