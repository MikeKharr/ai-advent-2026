...
/**
 * Запрос к агенту. Слоты лимитера резервируются до любого обращения к агенту
 * (I-4), включая создание диалога первым сообщением: запуск тратит деньги, и
 * отказ лимитера должен случаться раньше, а не позже.
 *
 * Слотов резервируется столько, сколько кругов проверки разрешено профилем
 * (ADR 2026-09-21-1747, п. 5): каждый круг сверх первого — ещё один платный
 * ответ и ещё одна платная проверка. Неиспользованные возвращаются по `end`
 * потока событий, где агент называет число сделанных кругов.
 */
...
async function handlePause(req, res) {
  ...
  if (!run?.id) return send(res, 409, { error: 'Запуск уже завершён', code: 'no_run' })
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || response.status === 409)
        return send(res, response.status, {
          error: json?.message ?? (response.status === 404 ? 'Запуск не найден' : 'Запуск уже завершён'),
          code: json?.code,
        })
      throw new Error(`агент ${response.status}`)
    }
