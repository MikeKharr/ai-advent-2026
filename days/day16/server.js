...
async function handleRpc(req, res) {
  ...
  // Байты уходят как есть. Тип содержимого проставляется РОВНО ТОГДА, когда
  // служба прислала его сама и в простой узнаваемой форме. Во всех прочих
  // случаях страница не получает типа вовсе, и таких случаев два:
  //   1) типа не было — так отвечает эндпоинт, когда прячется (404 без тела);
  //   2) тип был, но незнакомой формы — чужие параметры заголовка страница
  //      наружу не переписывает.
  //
  // Раньше на оба случая стояло умолчание `application/json`. Оно обещало
  // странице JSON, которого в ответе может не быть: на пустом теле — заведомо,
  // на незнакомом типе — как повезёт. Обещание типа — такое же утверждение о
  // содержимом, как переупаковка тела, и такое же ложное. Поэтому умолчания
  // нет ни в одном из двух случаев, а не только в первом.
