...
export function describe({ outcome, status, reason, bodyText = '', retryAfterSec }) {
  ...
  const notes = {
    ...
    // (решение владельца 2026-09-24). Строка про 401 описывала бы механизм,
    ...
    // Причину 404 страница НЕ называет, и это замер, а не осторожность:
    // служба отвечает так и на запрос без годного ключа, и когда отказов с
    // адреса набралось слишком много (mcp/src/service.js, `refusalsExhausted`
    // ДО сверки ключа). Проверено живьём — после 60 проб «без ключа» рабочая
    // команда с годным ключом тоже получает 404. Строка «ключ не подошёл»
    // была бы в этот момент ложью.
    404: '404 — эндпоинт отвечает так, будто его здесь нет: ни ошибки, ни подсказки, что по этому адресу что-то есть. Так он отвечает на запрос без годного ключа и так же — когда таких запросов пришло слишком много. Это защита, а не поломка.',
