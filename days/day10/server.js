// День 10: чат дня 9 со стратегиями памяти и ветками (ADR 2026-09-14-0447).
// Устройство дня 7: день отвечает за публичный адрес, лимитер и сессию; память
// диалога живёт у агента, стратегия и её параметры страница шлёт во входе
// запуска. Новое против дня 9: параметры стратегии уходят агенту и при чтении
// сессии (счётчик обязан описывать текущий режим), плюс переключение головы ветки.
//
...
// Читаемое имя сессии. Наружу отдаётся не идентификатор — он ключ к
// переписке, — а метка, выведенная из него необратимо (ADR 2026-09-09-2134).
...
async function handleChat(req, res) {
  ...
  try {
    ...
    const { response, json } = await callAgent(
      `/v1/sessions/${session.sessionId}${contextQuery(req)}`,
    )
 ...
 * указанного сообщения (ADR 2026-09-14-0447, п. 8.3). Принадлежность сообщения
 * сессии проверяет агент; идентификатор сессии день берёт только из cookie,
 * поэтому чужую ветку страница не адресует даже подбором номера.
 */
async function handleHead(req, res) {
  ...
  try {
    ...
    return send(res, 400, { error: 'тело не JSON' }, session.headers)
  }
  if (!Number.isInteger(body?.messageId))
    return send(res, 400, { error: 'messageId должен быть целым числом' }, session.headers)

  try {
    const { response, json } = await callAgent(`/v1/sessions/${session.sessionId}/head`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ messageId: body.messageId }),
    })
    // Отказы агента доходят его словами и его кодом: 404 — сообщение не этой
    // сессии, 409 — идёт запуск. Страница показывает их по-разному.
    if (response.status === 404 || response.status === 409)
      return send(
        res,
        response.status,
        { error: json?.message ?? 'Ветку переключить не удалось', code: json?.code },
        session.headers,
      )
    if (!response.ok) throw new Error(`агент ${response.status}`)
    return send(res, 200, { head: json?.head ?? null }, session.headers)
  } catch (error) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/chat/head' && req.method === 'PUT') return handleHead(req, res)
...
if (process.env.NODE_ENV !== 'test') {
  server.listen(env.PORT, () => console.log(`день 10 слушает :${env.PORT}`))
}
