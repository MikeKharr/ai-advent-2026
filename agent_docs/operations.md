...
2. Положить на сервер в `deploy/router.env` строкой `GROQ_API_KEY=gsk_…`
   и перезапустить: `docker compose up -d router`.
3. Роутер ходит в OpenAI-совместимый `POST /openai/v1/chat/completions`.
   ...
   (при `none` не отправляется вовсе — модели без его поддержки отвечают 400).
4. Модель по умолчанию — `meta-llama/llama-prompt-guard-2-22m`: это
...
5. Цена классификатора в `providers.json` задана с запасом вверх
   ($0.03 за 1M в обе стороны): на странице модели она указана только для
   превью. Завышенная цена запирает лимит расхода раньше, а не позже.
