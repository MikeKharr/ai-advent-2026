...
| `GET /v1/spend` | админ | расход и остатки по приложениям за текущие сутки UTC |
...
провайдера (приложение, класс, провайдер, токены, цена, исход), без промптов.
Суточные суммы восстанавливаются из него при перезапуске.
