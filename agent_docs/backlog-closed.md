...
- [x] 2026-09-11 — `detect` в `deploy.yml` считает от head sha последней успешной выкатки по push, а не от `HEAD^`; без базы или при базе не предке — все единицы (`.github/scripts/deploy-units.sh`, класс A). Правило «не мержить, пока выкатка ждёт» в `operations.md` снято. → `operations.md`, раздел «Атлас проекта»
