...
PR 2 из трёх по решению о датах в именах атомарных документов (ADR в
PR #109, принят владельцем 2026-09-11: переименовать + починить правило,
...
- Ссылка на сам ADR — только номером PR: файла в `main` пока нет, и
  `atlas --check` уронил бы сборку на неразрешённой цитате.
