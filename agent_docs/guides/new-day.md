...
**Ручной шаг ровно один раз на каждый новый день:** после первой публикации сделать пакет
`advent-dayN` в GHCR публичным. Сервер тянет образы анонимно — `docker login` на нём нет,
и первый `docker compose pull` приватного пакета уронит деплой.
