...
2. **`deploy/Caddyfile`** — две строки рядом с блоком дня 1:
   ...
   сверяет контрольную сумму `deploy/Caddyfile` до и после `git pull` и, если она
   изменилась, проверяет конфигурацию на сервере и перезагружает её без простоя
   (`caddy reload`) — до шага проверки живого адреса.
