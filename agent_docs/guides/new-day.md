...
2. **`deploy/Caddyfile`** — две строки рядом с блоком дня 1:
   ...
   workflow деплоя падает. **Ручного шага для этого больше нет:** выкатка сама
   сверяет контрольную сумму `deploy/Caddyfile` с последней удачно применённой и
   при расхождении проверяет конфигурацию на сервере и перезагружает её без
   простоя (`caddy reload`) — до шага проверки живого адреса.
...
**Ручных шагов первого деплоя не осталось** (см. шаг 2): новый маршрут применяет сама
выкатка. Если она всё же упала на проверке живого адреса, смотреть её шаг
перезагрузки входа в логе — маршрут не применён молча быть не может.
