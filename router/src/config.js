...
export const PROFILE_DEFAULTS = {
  laptop: { loadMs: 20_000, promptEvalTps: 200, genTpsFloor: 6.4, margin: 1.25, minMs: 0 },
  server: { loadMs: 0, promptEvalTps: 500, genTpsFloor: 25, margin: 1.25, minMs: 0 },
  cloud: { loadMs: 0, promptEvalTps: 2000, genTpsFloor: 40, margin: 1.25, minMs: 60_000 },
}
...
export function validateProviders(providers, env) {
  ...
  for (const p of providers) {
    ...
    for (const f of ['id', 'kind', 'tier', 'baseUrl', 'model', 'profile', 'jurisdiction'])
    ...
    if (!PROFILES.includes(p.profile)) fail(`${where}: неизвестный profile ${p.profile}`)
    try {
      new URL(p.baseUrl)
    } catch {
...
export function validateClasses(classes, providers) {
  if (!classes || typeof classes !== 'object') fail('classes: ожидается объект')
  if (!classes.other) fail('classes: класс other обязателен — он ловит неизвестные классы')
  for (const [name, c] of Object.entries(classes)) {
    ...
    const capable = candidates.filter((p) => capabilityFit(p, c, c.thinking, c.dataClass, 0).ok)
    if (capable.length === 0)
      fail(`${where}: ни один провайдер не удовлетворяет требованиям (ярусы, возможности, данные)`)
  }
}

export function validateApps(appsConfig, env) {
  if (!appsConfig || typeof appsConfig !== 'object') fail('apps: ожидается объект')
  ...
  for (const a of appsConfig.apps) {
    ...
    ids.add(a.id)
    if (!env[a.secretEnv]) fail(`${where}: переменная ключа ${a.secretEnv} не задана`)
...
export function capabilityFit(p, cls, level, dataClass, inputTokens, extraRequires = []) {
  const required = [...(cls.requires ?? []), ...extraRequires]
  for (const cap of required)
    if (!p.capabilities.includes(cap)) return { ok: false, reason: `нет возможности ${cap}` }
  if (level !== 'none' && p.thinking[level] === undefined)
    return { ok: false, reason: `уровень размышлений ${level} не поддерживается` }
  ...
  if (inputTokens > p.contextWindow)
    return { ok: false, reason: `вход ${inputTokens} токенов больше окна ${p.contextWindow}` }
  return { ok: true }
}
