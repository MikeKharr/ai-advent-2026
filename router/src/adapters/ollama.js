...
export async function call(
  ...
  if (!response.ok) {
    const error = new Error(`ollama ${response.status}: ${json?.error ?? 'unknown'}`)
  ...
  return {
    text: json.response ?? '',
    usage: { inputTokens: json.prompt_eval_count ?? 0, outputTokens },
    metrics: {
      ...
      tokPerSec: evalMs > 0 ? Math.round((outputTokens / evalMs) * 1000 * 10) / 10 : null,
    },
