...
export function createService({
  ...
  const providerOf = (id) =>
    router.providers().find((p) => `${p.id}#${p.revision}` === id);
  const priceCeiling = Math.max(
    0,
    ...router
      .providers()
      .map((p) =>
        Math.max(p.price?.inputPerMTok ?? 0, p.price?.outputPerMTok ?? 0),
      ),
  );
  ...
  const authApp = (req) => {
    ...
    for (const app of apps)
      if (safeEqual(token, env[app.secretEnv])) return app;
    return null;
  };
  ...
  function budgetLeft(app, at) {
    ...
    return {
      ...
      costUsd: l.dailyCostUsd
        ? Math.max(0, round(l.dailyCostUsd - s.costUsd))
        : null,
      resetAt: resetAt(at),
    };
  }
  ...
  function exhausted(app, at, estimateTokens) {
    ...
    if (
      l.dailyCostUsd &&
      s.costUsd + (need / 1e6) * priceCeiling > l.dailyCostUsd
    )
  ...
  async function handleRoute(req, res) {
    ...
    for (const attempt of result.attempts ?? []) {
      ...
      if (!p)
        throw new Error(
          `учёт: провайдер ${attempt.provider} неизвестен реестру`,
        );
      ...
      ledger.record({
        ...
        fallback:
          result.fallback?.from != null &&
          attempt.provider !== result.fallback.from,
...
function parseBody(raw) {
  ...
  if (!body || typeof body !== "object")
    throw new Error("тело должно быть объектом");
  ...
  if (
    ...
    !(Number.isInteger(body.budgetMs) && body.budgetMs >= 1000)
  )
