...
// Исходы, где до генерации дело не дошло и платить не за что: транспорт
// (`unreachable`), 429 (`busy`), 4xx (`rejected`) и 5xx, включая 529
// overloaded (`server_error`) — обратное давление поставщика, то же «не
// сейчас», что и 429. Такая попытка идёт в книгу с нулём на обоих концах.
export const NOT_REACHED = new Set(['unreachable', 'busy', 'rejected', 'server_error'])
// Исходы, где запрос ушёл в сеть, провайдер генерировал и тарифицирует
// сгенерированное, а измерения не вернулось: прерывание — обрыв клиента и
// потолок budgetMs (`aborted`), дедлайн вызова (`timeout`), успешный код с
// неразборным телом (`bad_response`). Кто дёрнул стоп-кран, на счёт
// провайдера не влияет, поэтому выход у них идёт по оценке, как и вход.
// Пустой ответ при 200 (`empty`) сюда не входит: там измерение провайдера
// есть, и ноль на выходе — его цифра, а не допущение.
export const OUTPUT_ESTIMATED = new Set(['aborted', 'timeout', 'bad_response'])
...
export function createService({
  ...
  async function handleRoute(req, res) {
    ...
    // по оценке входа, с пометкой; кроме исходов, где до генерации дело не
    // дошло (NOT_REACHED): они в журнале с нулём. У попытки, оборвавшейся на
    // ходу (OUTPUT_ESTIMATED), по оценке идёт и выход: провайдер тарифицирует
    // сгенерированное независимо от того, кто дёрнул стоп-кран. Оценка —
    // верхняя граница: тот же потолок выхода, что лимит зарезервировал под
    // попытку до вызова. Она завышает, поэтому попадает в отчёт с пометкой
    // `estimated`.
