...
export function createRouter({
  ...
  const bump = (p, field) => {
    ...
    if (!stats.has(k))
      stats.set(k, { attempts: 0, ok: 0, failed: 0, skipped: 0 });
    stats.get(k)[field] += 1;
  };
  ...
  function resolveThinking(cls, requested) {
    if (requested === undefined || requested === cls.thinking)
      return { level: cls.thinking };
    if (cls.locked)
      return { error: `класс закреплён за уровнем ${cls.thinking}` };
  ...
  async function route(req) {
    ...
    if (
      DATA_RANK[dataClass] === undefined ||
      DATA_RANK[dataClass] < DATA_RANK[cls.dataClass]
    )
      return refuse(
        "refused",
    ...
    if (strict && schema === null)
      return refuse(
        "refused",
        `класс ${taskClass} требует schema в запросе`,
        [],
      );

    const inputTokens =
      estimateTokens(req.input) + estimateTokens(req.system ?? "");
    ...
    for (const p of capable) {
      ...
      if (attempt.outcome === "aborted" || attempt.outcome === "truncated")
        break;
    }
    const result = refuse(
      "all_failed",
  ...
  async function tryProvider(
    ...
    try {
      ...
      if (error.status === 429) {
        ...
        return done(
          "busy",
          `429, занят${error.retryAfterMs ? ` на ${error.retryAfterMs} мс` : ""}`,
        );
      }
      if (isUnreachable(error)) {
        ...
        return done(
          "unreachable",
          `недоступен: ${error.cause?.code ?? error.message}`,
        );
      }
      if (
        ctrl.signal.aborted ||
