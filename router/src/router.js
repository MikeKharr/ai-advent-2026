...
export function createRouter({
  ...
  async function tryProvider(
    ...
    const done = (outcome, reason, extra = {}) => ({
      ...
      // То же для выхода: прерванный запрос провайдер тарифицирует целиком,
      // а usage вместе с ответом не приходит. Берём верхнюю границу —
      // ровно ту величину выхода, которую лимит зарезервировал под эту
      // попытку до вызова (`estimateRequest`): потолок приложения тогда
      // не протекает. Это не обязательно `max_tokens` ушедшего запроса:
      // адаптер может поднять его под бюджет размышлений провайдера.
