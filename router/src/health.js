...
export function createHealth({ now = Date.now } = {}) {
  ...
  const get = (p) => {
    const k = key(p)
    if (!state.has(k))
      state.set(k, { negativeUntil: 0, failures: 0, openUntil: 0, probing: false, busyUntil: 0 })
    return state.get(k)
  }
  ...
  return {
    ...
    unavailableReason(p) {
      ...
      if (s.busyUntil > t) return `занят до ${new Date(s.busyUntil).toISOString()} (429)`
      if (s.openUntil > t)
        return `предохранитель разомкнут до ${new Date(s.openUntil).toISOString()}`
      if (s.openUntil !== 0 && s.probing) return 'предохранитель полуоткрыт, проба уже идёт'
