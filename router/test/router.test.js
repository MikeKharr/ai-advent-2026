...
const CLASSES = JSON.parse(
  readFileSync(new URL("../config/classes.json", import.meta.url), "utf8"),
);
...
test("провайдер в отрицательном кэше пропускается без обращения", async () => {
  ...
  assert.equal(
    ...
    1,
    "второй раз к ноутбуку не ходили",
  );
});
...
test("три подряд неудачи размыкают предохранитель", async () => {
  ...
  assert.equal(
    calls.filter((c) => c.host === LAPTOP).length,
    3,
    "четвёртый раз не звали",
  );
  assert.match(
    result.reasons?.[0]?.reason ??
      router.health.unavailableReason(PROVIDERS[0]),
    /предохранитель/,
  );
});
...
test("при inflight ≥ maxConcurrency провайдер пропускается", async () => {
  ...
  assert.equal(
    second.fallback,
    null,
    "это пропуск по здоровью, а не фолбэк после вызова",
  );
  assert.equal(calls.filter((c) => c.host === LAPTOP).length, 1);
  release();
...
test("rank_news на self-hosted — отказ, а не понижение", async () => {
  ...
  assert.equal(
    calls.filter((c) => c.host === LAPTOP).length,
    0,
    "ноутбук без web_search не звали",
  );
  assert.deepEqual(
...
test("данные, не выходящие за периметр, при одном облаке — отказ", async () => {
  ...
  assert.equal(
    calls.filter(
      (c) => c.host === CLOUD && c.body.messages[0].content === "секрет",
    ).length,
    0,
  );
  assert.equal(
    result.reasons.find((r) => r.provider === "anthropic-haiku#1").stage,
    "capability",
  );
  assert.equal(
    result.reasons.find((r) => r.provider === "mac-qwen3#1").stage,
    "health",
  );
});
...
test("пустой 200 — неудача с фолбэком, в кэш не попадает", async () => {
  ...
  assert.equal(
    second.provider.id,
    "mac-qwen3",
    "пустой ответ не закэширован как недоступность",
  );
  assert.equal(calls.filter((c) => c.host === LAPTOP).length, 2);
});
...
test("провайдер и уровень размышлений есть в ответе, логе и ключе кэша", async () => {
  ...
  assert.equal(
    calls[0].body.think,
    true,
    "значение think из конфигурации провайдера",
  );
  assert.equal(
...
test("потолок вызывающего budgetMs прерывает вызов, но в предохранитель не идёт", async () => {
  ...
  assert.equal(
    router.health.unavailableReason(PROVIDERS[1]),
    null,
    "предохранитель не разомкнут",
  );
...
test("обрезанный, но разобравшийся объект схемного класса — успех с truncated", async () => {
  const { router } = setup({
    hosts: {
      [LAPTOP]: () =>
        httpJson(200, ollamaGenerate({ text: '{"a":1}', done: "length" })),
    },
  });
  const r = await router.route({
    taskClass: "extract_json",
...
test("429 с не-JSON телом — «занят», а не поломка", async () => {
  const { router } = setup({
    ...
    hosts: {
      [CLOUD]: () =>
        httpText(429, "<html>Too Many Requests</html>", { "retry-after": "2" }),
    },
  });
...
test("обрезание: свободный текст — успех с truncated, схемный класс — неудача без фолбэка", async () => {
  ...
  const strict = setup({
    hosts: {
      [LAPTOP]: () =>
        httpJson(200, ollamaGenerate({ text: '{"a":', done: "length" })),
      [CLOUD]: cloudOk,
    },
  });
  ...
  assert.equal(
    r2.attempts.length,
    1,
    "после обрезания схемного ответа второго не зовём",
  );
