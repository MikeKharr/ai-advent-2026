...
const cloudOk = () =>
  httpJson(200, anthropicMessage({ input: 100, output: 50 }));
const laptopOk = () =>
  httpJson(200, ollamaGenerate({ input: 100, output: 50 }));
...
test("неизвестный ключ приложения — 401, провайдер не вызывается", async () => {
  ...
  try {
    const res = await s.post(
      { taskClass: "summarize", input: "текст" },
      "wrong",
    );
...
test("вызов записывается в журнал, следующий видит уменьшенный остаток", async () => {
  ...
  try {
    const first = await (
      await s.post({ taskClass: "summarize", input: "текст" })
    ).json();
    ...
    assert.equal(
      "prompt" in lines[0] || "input" in lines[0],
      false,
      "промпт в журнал не пишется",
    );

    const second = await (
      await s.post({ taskClass: "summarize", input: "ещё" })
    ).json();
...
test("исчерпанный лимит токенов — budget_exceeded с датой сброса, провайдер не вызывается", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++)
      assert.equal(
        (await s.post({ taskClass: "summarize", input: "текст" })).status,
        200,
      );
...
test("суточные суммы восстанавливаются из журнала после перезапуска", async () => {
  ...
  try {
    ...
    const next = await (
      await b.post({ taskClass: "summarize", input: "текст" })
    ).json();
...
test("неудачный вызов без usage списывается по оценке входа", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      200,
      "по 100 токенов оценки входа на попытку",
    );
  } finally {
    await s.close();
  }
...
test("оба вызова при фолбэке считаются в лимит приложения, цена — по провайдеру", async () => {
  const s = await start({
    hosts: {
      [LAPTOP]: () =>
        httpJson(200, ollamaGenerate({ text: "", input: 100, output: 0 })),
      [CLOUD]: cloudOk,
    },
  });
  try {
...
test("класс вне списка приложения — 403; /v1/spend и /v1/metrics — только админу", async () => {
  ...
  try {
    assert.equal(
      (await s.post({ taskClass: "translate", input: "x" })).status,
      403,
    );
    ...
    assert.equal(
      (await s.post({ taskClass: "summarize", input: "" })).status,
      400,
    );
    assert.equal(
      (await s.post({ taskClass: "summarize", input: "x", budgetMs: 5 }))
        .status,
      400,
    );
  } finally {
