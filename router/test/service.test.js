  ...
  apps: [
    {
      ...
      limits: { dailyTokens: 500, dailyCostUsd: 1 },
    },
  ],
}
...
test('вызов записывается в журнал, следующий видит уменьшенный остаток', async () => {
  ...
  const s = await start({ hosts: { [LAPTOP]: laptopOk, [CLOUD]: cloudOk }, file })
  try {
    const first = await (await s.post({ taskClass: 'summarize', input: 'текст' })).json()
    assert.equal(first.ok, true)
    assert.equal(first.app, 'smoke')
    assert.deepEqual(first.budgetLeft.tokens, 500 - 150)
    ...
    assert.equal(second.budgetLeft.tokens, 500 - 300)
  } finally {
    await s.close()
  }
})
...
test('суточные суммы восстанавливаются из журнала после перезапуска', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'ledger-'))
  const file = join(dir, 'ledger.jsonl')
  const a = await start({ hosts: { [LAPTOP]: laptopOk, [CLOUD]: cloudOk }, file })
  await a.post({ taskClass: 'summarize', input: 'текст' })
  await a.post({ taskClass: 'summarize', input: 'текст' })
  await a.close()

  const b = await start({ hosts: { [LAPTOP]: laptopOk, [CLOUD]: cloudOk }, file })
  try {
    ...
    assert.equal(next.budgetLeft.tokens, 500 - 450)
  } finally {
    await b.close()
  }
})
...
test('приложение без лимитов — крах на старте', () => {
  const apps = { ...APPS, apps: [{ id: 'free', secretEnv: 'APP_KEY_SMOKE', classes: ['other'] }] }
  assert.throws(
    () => loadConfig({ providers: PROVIDERS, classes: CLASSES, apps, env: ENV }),
    /лимит/,
  )
  const zero = { ...APPS, apps: [{ ...APPS.apps[0], limits: { dailyTokens: 0 } }] }
  assert.throws(
    () => loadConfig({ providers: PROVIDERS, classes: CLASSES, apps: zero, env: ENV }),
    /лимит/,
  )
})
...
test('оба вызова при фолбэке считаются в лимит приложения, цена — по провайдеру', async () => {
  ...
  try {
    ...
    assert.equal(body.budgetLeft.tokens, 500 - 100 - 150)
...
test('класс вне списка приложения — 403; /v1/spend и /v1/metrics — только админу', async () => {
  const s = await start({ hosts: { [LAPTOP]: laptopOk, [CLOUD]: () => unreachable('ENOTFOUND') } })
  try {
