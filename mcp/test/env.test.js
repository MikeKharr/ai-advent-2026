...
test('умолчания: порт 8083 и окна лимитера', () => {
  ...
  // Потолок отказов нарочно не равен рабочим окнам: другая величина и
  // другой смысл (см. комментарий в src/env.js).
  assert.equal(env.REFUSALS_PER_HOUR, 60)
  assert.notEqual(env.REFUSALS_PER_HOUR, env.RATE_LIMIT_PER_HOUR)
})
